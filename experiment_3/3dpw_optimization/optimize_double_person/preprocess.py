...
def get_label(img_id=0, visualize=False):
    ...
    mask = cv2.imread(os.path.join(dst_dir, "courtyard_dancing_00_mask/mask",
                                  'image_%s.png' % str(img_id).zfill(5)))
    instance_a = cv2.imread(os.path.join(dst_dir, "courtyard_dancing_00_mask/instance",
                                       str(1), 'image_%s.png' % str(img_id).zfill(5)))
    instance_b = mask - instance_a

    label['img'] = img
    label['mask'] = mask[:, :, 0] / 255
    label['instance_a'] = instance_a[:, :, 0] / 255
    label['instance_b'] = instance_b[:, :, 0] / 255
    ...
    label['instance_a'] = label['instance_a_crop']
    label['instance_b'] = label['instance_b_crop']
