...
def preprocess_opt():
    ...
    opt.exp_name = opt.exp_name + '_id_[%g,%g]' % (opt.image_id_range[0], opt.image_id_range[1])
    ...
    if opt.pose_reg_weight > 0:
        opt.exp_name += '_pos_%g' % opt.pose_reg_weight
    if opt.kp2d_weight > 0:
        opt.exp_name += '_kp2_%g' % opt.kp2d_weight
    if opt.collision_weight > 0:
        opt.exp_name += '_col_%g' % opt.collision_weight
    if opt.touch_weight > 0:
        opt.exp_name += '_tou_%g' % opt.touch_weight
    if opt.pose_prior_weight > 0:
        opt.exp_name += '_pri_%g' % opt.pose_prior_weight
    if opt.shape_consistency_weight > 0 and opt.num_img > 1:
        opt.exp_name += '_sc_%g' % opt.shape_consistency_weight
    if opt.kp3d_consistency_weight > 0 and opt.num_img > 1:
        opt.exp_name += '_j3c_%g' % opt.kp3d_consistency_weight
    if opt.global_pose_consistency_weight > 0 and opt.num_img > 1:
        opt.exp_name += '_gpc_%g' % opt.global_pose_consistency_weight
...
def load_check_point(path, para):
